...
const page = () => {
  ...
  return (
    <div className="h-screen w-full flex flex-col bg-black overflow-hidden font-mono relative">
      <AnimatePresence>
        {reveal && (
          <motion.div
            key="overlay"
            ...
            className="absolute h-full w-full z-50 flex justify-center items-center bg-black/5 backdrop-blur-md"
            onClick={() => setReveal(false)}
          >
            <motion.div
              initial={{ rotateY: -35, opacity: 0, filter: "blur(20px)" }}
              animate={{ rotateY: 0, opacity: 1, filter: "blur(0px)" }}
              exit={{ rotateY: -35, opacity: 0, filter: "blur(20px)" }} // reverse anim
              transition={{ duration: 0.4, ease: "easeInOut" }}
              className="bg-blue-500 text-white px-10 py-5 rounded-lg shadow-lg text-xl transform-gpu origin-left z-[999]"
            >
      ...
      <div className="h-full grid-col-1 max-w-[54rem] mx-auto flex items-center flex-col justify-center gap-16 border-x border-neutral-900">
